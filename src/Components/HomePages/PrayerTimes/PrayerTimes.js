...
const PrayerTimes = () => {
  ...
  return (
    <div className={`w-full flex justify-center items-center py-3 ${dark_mode ? `bg-dark_main_bg_color` : `bg-dark_second_bg_color`}`}>
      <div className="container flex flex-wrap justify-center items-center gap-6">
        <div
          className={`flex flex-wrap justify-center items-center gap-9 `}
        >
          {/* header */}
          <div className={`w-full flex justify-center items-center md:mb-5  `}>
            <div
              className={`flex justify-center items-center gap-4 border-b-2 py-4 ${dark_mode ? `border-border_color` : `border-main_hover_color`}`}
            >
              ...
              <div className="flex justify-center items-center">
                <h2
                  className={`w-full text-xl sm:text-3xl font-bold ${dark_mode ? `text-white` : `text-main_hover_color`}`}
                >
                  {" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-1 h-full ">
            {/* Location & Time */}
            <div className={`md:col-span-1 xl:col-span-2 flex flex-wrap justify-center items-center gap-6 px-6 pb-6 rounded-lg ${dark_mode ? `bg-dark_main_bg_color border border-border_color` : `bg-dark_second_bg_color`} `}>
              <h3 className={`w-full text-3xl font-bold text-start ${dark_mode ? `text-white` : `text-black`}`}>
                {" "}
                مصر, المنيا{" "}
              </h3>
              <p className={`w-full ${dark_mode ? `text-white` : `text-teal-600`} text-lg md:text-2xl font-bold`}>
                الساعة:{" "}
                <span className="w-full text-orange-600 font-bold">{time}</span>
              </p>
              <p className={`w-full ${dark_mode ? `text-white` : `text-teal-600`} md:text-xl font-bold`}>
            ...
            {/* Prayer Countdown */}
            <div className={`md:col-span-1 xl:col-span-1 flex flex-wrap justify-center items-center gap-6 p-6 text-white rounded-lg shadow-lg ${dark_mode ? `bg-dark_main_bg_color border border-border_color`: `bg-main_hover_color `}`}>
              ...
              <div className="w-full flex flex-wrap justify-center items-center gap-3 font-bold text-center">
                <span className={`w-full text-start text-xl p-2`}>   متبقي على صلاة {getPrayerName(nextPrayer.name)} </span>
                <div className={`w-full flex justify-center items-center gap-3 text-3xl p-3 ${dark_mode ? `bg-dark_main_bg_color border border-border_color` : `bg-dark_second_bg_color`} text-black text-center rounded-xl`}> 
                  <span className={`text-white p-2 px-3 ${dark_mode ? `bg-dark_main_bg_color border border-border_color hover:bg-[#666]` : `bg-main_hover_color hover:bg-[#24696d]`} rounded-lg duration-500 cursor-pointer`}> {remainingSecond} </span>
                  <span className={`text-white p-2 px-3 ${dark_mode ? `bg-dark_main_bg_color border border-border_color hover:bg-[#666]` : `bg-main_hover_color hover:bg-[#24696d]`} rounded-lg duration-500 cursor-pointer`}> {remainingMinute} </span>
                  <span className={`text-white p-2 px-3 ${dark_mode ? `bg-dark_main_bg_color border border-border_color hover:bg-[#666]` : `bg-main_hover_color hover:bg-[#24696d]`} rounded-lg duration-500 cursor-pointer`}> {remainingHour} </span>
                  {/* {remaining}  */}
            ...
            {loading ? (
              ...
              <div className={`w-full overflow-x-auto shadow-lg rounded-lg`}>
                <table className="w-full border border-gray-300 text-center">
                  <thead className={`hidden lg:table-header-group ${dark_mode ? `bg-dark_main_bg_color`: `bg-gray-200 `}`}>
                    <tr>
                      {Object.keys(timings.timings).map((prayer, index) => (
                        <th
                          key={index}
                          className={`text-center border  py-3 px-2 text-lg font-bold ${dark_mode ? `border-border_color text-white` : `border-gray-300 text-black`}`}
                        >
                          {
                            <>
                              ...
                              <span className={`block`}>{getPrayerName(prayer)}</span>
                            </>
                          }
                        </th>
                      ))}
                  ...
                  <tbody>
                    <tr className="hidden lg:table-row">
                      {Object.keys(timings.timings).map((prayer, index) => (
                        <td
                          key={index}
                          className={`border ${dark_mode ? `border-border_color text-white` : `border-gray-300 text-black`} p-3 text-lg`}
                        >
                      ...
                      <span className="w-full flex justify-center items-center gap-2 text-lg font-semibold border-b-2 ">
                        {
                          <>
                            ...
                            <span className={`py-3`}> {getPrayerName(prayer)} </span>
                          </>
                        }
                      </span>
